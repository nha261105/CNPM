...
export const checkIn = async (req: Request, res: Response) => {
    ...
    const attendanceLog = await attendanceLogService.checkIn({ schedule_id, student_id});
...
export const checkOut = async (req: Request, res: Response) => {
    ...
    const attendanceLog = await attendanceLogService.checkOut({ schedule_id, student_id });
    return res.json({ ok: true, data: attendanceLog });
}